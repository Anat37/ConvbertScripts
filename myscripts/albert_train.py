...
#config = AlbertConfig(hidden_size=768, num_attention_heads=12, intermediate_size=3072, attention_probs_dropout_prob=0, num_hidden_groups=4, num_hidden_layers=4)
config = AlbertConfig.from_pretrained('albert-base-v2')
config.save_pretrained(model_dir)
#model = ConvbertForPreTraining(config)
#model = ConvbertForPreTraining.from_pretrained(get_last_checkpoint(output_dir))
model = AlbertForPreTraining.from_pretrained('albert-base-v2')
model.save_pretrained(model_dir)


train_dataset = SOPDataset(directory='E:/ConvbertData/text_data2/cache', batch_size=40, tokenizer=tokenizer, mlm_probability=0.1)
